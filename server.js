...
if (!CLIENT_ID) {
  console.error('❌ Error: CLIENT_ID is missing in environment variables');
  process.exit(1);
}

// 環境変数のパース（エラーハンドリング強化）
const config = {
  allowedUserIds: (() => {
    try {
      return process.env.ALLOWED_USER_IDS ? JSON.parse(process.env.ALLOWED_USER_IDS) : [];
    } catch (e) {
      console.error('❌ ALLOWED_USER_IDS parse error:', e.message);
      return [];
    }
  })(),
  lurerUserIds: (() => {
    try {
      return process.env.LURER_USER_IDS ? JSON.parse(process.env.LURER_USER_IDS) : [];
    } catch (e) {
      console.error('❌ LURER_USER_IDS parse error:', e.message);
      return [];
    }
  })()
};
  ...
  try {
    if (interaction.commandName === 'create-lottery') {
      ...
      try {
        ...
        console.error('❌ parseJSTDate error:', error.message);
    ...
    if (!interaction.replied) {
      await interaction.reply({ content: '❌ エラーが発生しました。管理者に連絡してください。', flags: MessageFlags.Ephemeral }).catch(console.error);
    }
  }
});
...
async function registerGlobalCommands() {
  ...
  try {
    ...
    console.error('❌ グローバルコマンド登録エラー:', error.message);
  }
}
